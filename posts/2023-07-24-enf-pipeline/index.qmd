...
There are more files but these are the main one's you need to worry about.

The "config_files" folder holds all of the configuration .json files. These are the parameters that are input into the python script so you can run enformer on any region and individual you want. We will go more in-depth about the config files later

the "metadata" folder holds the intervals you want to predict on in the form of .txt files.

the "scripts" folder contains the main python script with all the code for running enformer, as well as a modules folder which has more helper functions for the main script

Finally the "software" folder contains the yaml file with all the packages and channels the python script imports and uses.

The only thing you have to do is supply your own configuration.json and intervals.txt file, and call it in the command line.
...
-   prediction_id: an id to separate your predictions under the same prediction_data_name
-   use_parsl: should be true so you can run in parallel using all of polaris' GPUs
-   output_dir: name of folder where your predictions will go
